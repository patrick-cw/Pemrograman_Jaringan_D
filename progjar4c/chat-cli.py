...
class ChatClient:
    def __init__(self):
        ...
        self.enter = Button(self.loginGUI,
                            ...
                            command=lambda: self.proses("auth"+" "+self.inputUsername.get()+" "+self.inputPassword.get())
                            )
    ...
    def inbox(self):
        ...
        if result['status']=='OK':
            ...
            self.textCons.insert(END,format(json.dumps(result['pengirim'])+": "+format(json.dumps(result['messages']))+"/n"))
